...
public class Field {
   ...
   private double objectFriction, kineticFrictionCoefficient, height, width;
   private int difficulty, tries, maxTry;
    ...
    * @param prefWidth is the preferred width of the field.
    * @param prefHeight is the preferred height of the field.
   ...
   private void setFriction() {
      if( difficulty == 0 ){
         kineticFrictionCoefficient = 0.9;
      }
      else if( difficulty == 1 ) {
         kineticFrictionCoefficient = 1.3;
      }
      else {
         kineticFrictionCoefficient = 1.7;
      }
   }
   /**
    * sets the maximum number of tries according to the difficulty
    */
   private void setTry(){
	   if( difficulty == 0 ){
	         maxTry = 5;
   ...
   public void slowDown( Coin c ) {
      ...
      if ( difficulty == 0 ) {
         objectFriction =( 0.1 + Math.abs( 1 - ( c.getX() - width/2 ) ) / (width / 2) ) * kineticFrictionCoefficient * 
      ...
      if ( Math.abs(c.getVelocityX()) < objectFriction
    		  && c.getVelocityY() < objectFriction ){
    	  c.setSpeed(0, 0);
      }
      isInside(c);
      isCrushed(c);
    ...
    */
   public boolean isInsideField( Coin c)
   {
   ...
   /**
    * Returns maximum number of tries
    * @return maxTry
    */
   public int getTry(){
	   return maxTry;
